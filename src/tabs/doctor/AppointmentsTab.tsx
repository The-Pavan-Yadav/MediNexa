...
import type { MhdUser, Appointment, ApptStatus } from '../../lib/types';
...
export default function AppointmentsTab({ doctorData }: { doctorData: MhdUser }) {
  ...
  const setStatus = async (a: Appointment, status: ApptStatus) => {
    try {
      await updateDoc(doc(db, 'appointments', a.id), { status });
      if (status === 'cancelled' || status === 'rejected') await deleteDoc(doc(db, 'slots', slotKey(a.doctorId, a.date, a.time))).catch(() => { /* ignore */ });
      let title = `Appointment ${status}`;
      if (status === 'completed') title = '✅ Appointment completed';
      else if (status === 'cancelled' || status === 'rejected') title = `🚫 Appointment ${status}`;
      else if (status === 'confirmed') title = '✅ Appointment confirmed';

      await notify(a.patientId, title, `Dr. ${doctorData.name} marked ${fmtD(a.date)} ${a.time} as ${status}`);
      toast(`Marked ${status}`);
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-6 pb-12">
      ...
      <FilterPills filters={['All', 'Upcoming', 'Confirmed', 'Completed', 'Cancelled', 'Rejected']} value={filter} onChange={setFilter} />
            ...
            {list.map((a) => (
              <div key={a.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 hover:bg-stripe transition-colors">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {a.status === 'upcoming' && (
                    <>
                      <button onClick={() => setStatus(a, 'confirmed')} className="text-[12px] font-medium text-primary border border-primary-bd px-3 py-1.5 rounded-[4px] hover:bg-app transition-colors">Confirm</button>
                      <button onClick={() => setStatus(a, 'rejected')} className="text-[12px] font-medium text-danger border border-danger-bd px-3 py-1.5 rounded-[4px] hover:bg-danger-bg transition-colors">Reject</button>
                    </>
                  )}
                  {a.status === 'confirmed' && (
                    <>
